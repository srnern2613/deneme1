...
### Üçüncü tur — `draconic_lingua_ui_iyilestirme_listesi_2.md` (18.09.2026, proje sahibinin gerçek testinden çıkan yeni liste)

Proje sahibi Sıra 1-10'u kendi cihazında test etti; sonuç iki yeni P0 (biri muhtemelen kendi P1-8 düzeltmemin yan etkisi) ve N-1…N-13 arası tamamen yeni bir madde grubu. Bu turda **sormadan** yapılan işler:

- [x] **P0-A (Lobi karusel overflow) çözüldü.** ✅ 18.09.2026 — Kök neden: Sıra 10'da başlığı `maxLines: 1→2` yaptığım değişiklik, kartın sabit `SizedBox(height: 115)` bütçesini aşıp 1px `RenderFlex` taşması üretti. `main.dart`: yükseklik 115→132, `BookCover` ile başlık arasındaki `Spacer()` (merkezlemeyle çakışan, öngörülemez esneme kaynağı) sabit `SizedBox(height: 8)` ile değiştirildi.
- [x] **P0-B (Kitaplık'ta iç içe scroll) incelendi — kodda üreme yok.** ✅ 18.09.2026 — `library_screen.dart` tek `Expanded` + `ListView.separated` kullanıyor (dış scroll yok, `shrinkWrap`/`NeverScrollableScrollPhysics` yok, ikinci bir `ListView`/`GridView`/`Scrollbar` yok). Tarif edilen "iki scrollbar + büyük boşluk" belirtisi bu koddan üretilemiyor — Flutter hot reload'un yapısal Column/Expanded değişikliklerinde bıraktığı hayalet render-object belirtisiyle eşleşiyor. **Öneri: bir sonraki testte tam yeniden başlatma (hot restart / durdur-çalıştır) yapıp tekrar bak; hâlâ görünüyorsa hangi build/commit'te olduğunu söyle.**
- [x] **P0-C (Sezon XP sahteydi) çözüldü — gerçek haftalık sayaç eklendi.** ✅ 18.09.2026 — Doğrulandı: "SEZON XP" etiketi vardı ama `leaderboard_screen.dart` hâlâ `getTotalXp()`'yi (header'la aynı `user_total_xp` kaynağı) okuyordu, hiç ayrı bir hafta sayacı yoktu. `xp_shop_service.dart`'a `season_xp` + `season_xp_week` (Pazar sınırlı hafta anahtarı) eklendi: `getSeasonXp()` hafta değiştiyse otomatik sıfırlıyor, `addXp()` her XP kazanımında hem toplamı hem sezonu güncelliyor. `leaderboard_screen.dart` artık `getSeasonXp()` okuyor — etiket artık gerçek veriye karşılık geliyor.
- [x] **N-2 (3 renkte PREMIUM rozeti) çözüldü.** ✅ 18.09.2026 — `flashcards_screen.dart`'taki üç PREMIUM satırı (Ters Test, Sadece Dinleme, Hız Turu) artık üçü de amber.
- [x] **N-3 (ÜCRETSİZ rozeti tersten okunuyordu) çözüldü.** ✅ 18.09.2026 — "Boşluk Doldurma" artık kardeşleriyle aynı dilde bir XP rozeti (`+8 XP`) taşıyor; "ücretsiz" artık sadece PREMIUM rozetinin YOKLUĞUYLA anlaşılıyor, tek satırlık özel bir "ÜCRETSİZ" etiketi diğer satırları yanlışlıkla ücretli gösterip duruyordu.
- [x] **N-4 (Arena pratik ikonları sistem dışı 5 renk) çözüldü.** ✅ 18.09.2026 — Hızlı Test/SRS Hafıza/Eşleştirme/Dinle & Yaz/Boşluk Doldurma artık beşi de indigo ailesinde, beş farklı ton (`#A5B4FC`→`#4338CA`) ile ayrışıyor — hepsi hafıza/pratik içeriği olduğu için C-2 gereği tek renk ailesi.
- [x] **N-5 (iki ekran de "Arena") kısmen çözüldü.** ✅ 18.09.2026 — Karar #8 uygulandı: alt bar 3. sekme etiketi "Kelimeler" → "Arena" (`main.dart`, ekran başlığıyla artık eşleşiyor). 4. sekmenin (`leaderboard_screen.dart`) başlığı "Lig Arenası" → "Sıralama" — artık iki ekranda da "Arena" kelimesi aynı anda geçmiyor.
- [x] **N-9 (Arena ayarları sheet'inde iki seçim rengi) çözüldü.** ✅ 18.09.2026 — "Tüm Kelimeler" chip'i cyan'dan amber'e; aynı sheet'teki "Tümü" chip'iyle artık tek seçim rengi.
- [x] **C-2 (liderlik "Sen" satırı indigo çerçeveli) çözüldü.** ✅ 18.09.2026 — `leaderboard_screen.dart`'ta "Eren (Sen)" satırının indigo (hafıza rengi) çerçevesi amber'e (eylem/vurgu rengi) taşındı — indigo artık her yerde tek bir anlam (hafıza) taşıyor.
- [x] **Yeni doküman kaydedildi.** ✅ 18.09.2026 — `docs/draconic_lingua_ui_iyilestirme_listesi_2.md` (proje sahibinin üçüncü tur listesi, tam metin) repoya eklendi.

**Bilinçli olarak bu turda YAPILMADI (kapsamı büyük / cihaz doğrulaması gerekiyor / ürün kararı gerekiyor):**
- **T-1'in gövdesi** (ekranlardaki ~600 ham hex'in `Theme.of(context).extension<DraconicTheme>()`'e taşınması) — bu, proje sahibinin doğrudan sorduğu "renkler neden değişmiyor" sorusunun tam cevabı: `ThemeController`/`DraconicTheme`/`AppButton`/`GlassPanel`/`DungeonCard` zaten context üzerinden doğru okuyor ve zindan↔parşömen anahtarı gerçekten çalışıp kalıcı oluyor — ama `main.dart`, `library_screen.dart`, `flashcards_screen.dart`, `leaderboard_screen.dart`, `profile_screen.dart` gibi ekranların Scaffold/kart/metin renkleri hâlâ `const Color(0xFF...)` ile ekrana gömülü, hiçbiri temaya bakmıyor. Bu yüzden anahtar değişiyor ama sahne aynı kalıyor. 20+ dosya, ~600 satır, görsel doğrulama olmadan (bu ortamda ekran görüntüsü alma imkânım yok) topluca değiştirmek P0-A'daki gibi yeni, görülmeyen taşma/kontrast hataları riski taşıyor — dosya dosya, test edilerek yapılması gerekiyor. **Sıra'daki 2. öncelik bu.**
- N-1 (rozet taksonomisi — kilit/fiyat/ödül üç ayrı görsel dile ayrılması, `AppBadge` bileşeni gerektiriyor), N-6/N-7/N-8 (okuyucu tema adlandırma + palet sayısı — ürün kararı), N-10 (Koleksiyon Vitrini kilitli karo koşulları), N-11 (okuyucu "0 Av" etiketi), N-12 (Profil "Görünüm" grup taşıma), N-13 (seri koşulu — ürün mantığı kararı), C-1/C-3/C-4'ün ekranlara uygulanması (T-1'e bağlı), A/I platform maddeleri (cihaz gerektiriyor).

### Dördüncü tur — proje sahibi ekran videosu izlendi + T-1'e başlandı (19.09.2026)

Proje sahibi kendi cihazından 41 saniyelik bir ekran videosu paylaştı; kareler tek tek incelendi. **Üçüncü turdaki tüm düzeltmeler (P0-A, P0-C, N-2/3/4/5/9, C-2) görüntüde doğrulandı, çalışıyor.** P0-B videoda hiç üremedi (Kitaplık sorunsuz kaydı) — muhtemelen gerçekten hot-reload artığıymış. Videoda ayrıca T-1'in gerekliliği doğrudan görüldü: Profil'de Zindan→Parşömen geçişinde buton seçimi değişiyor ama TÜM ekran koyu kalıyor.

Bu turda sorulmadan yapılanlar:
- [x] **C-1 (liderlik kartlarında üç farklı renkte "AL" butonu) çözüldü.** ✅ 19.09.2026 — `leaderboard_screen.dart`'taki Mikro-Meydan Okuma kartlarının "AL" butonu her kartın kendi `itemColor`'ını (yeşil/indigo/amber) taşıyordu; videoyla da doğrulanan bir C-1 ihlaliydi. Üçü de artık tek bir birincil görünüm — amber.
- [x] **Sözlük ekranındaki İngilizce etiket düzeltildi.** ✅ 19.09.2026 — `dictionary_screen.dart`'ta "Mastered Words" → "Kalıcı Hafıza" (Profil'deki aynı kavramla aynı terim). "Word Boss" bilinçli olarak dokunulmadı — uygulama genelinde (flashcards_screen, book_journey_screen, word_boss_battle_screen) tutarlı kullanılan, kasıtlı bir oyun terimi.
- [x] **T-1'in ilk dosyası tamamlandı: `library_screen.dart` artık tamamen tema-farkında.** ✅ 19.09.2026 — Scaffold arka planı, tüm kart zeminleri/çerçeveleri, tüm metin renkleri (`textPrimary`/`textSecondary`) ve tüm semantik vurgu renkleri (amber/emerald/teal/indigo) artık `Theme.of(context).extension<DraconicTheme>()!` üzerinden okunuyor — hiçbir `Color(0xFF...)` veya `Colors.white` kalmadı (grep ile doğrulandı). 8 build metodunun her birine kendi `theme` değişkeni eklendi (State sınıfı içinde `context` her yerde erişilebilir olduğu için ekstra parametre taşımaya gerek kalmadı). Bu, T-1'in geri kalan ~19 dosya için şablonu.
  **Bilinçli olarak dokunulmadı:** Kitap kapağı harf-monogramı paleti (`BookCover`, `primitives.dart` — ayrı, planlı iş, P1-4/C-2), rozet/ikon çeşitliliği taşıyan tek-seferlik dekoratif renkler (bunlar tema tokenı DEĞİL, kasıtlı çeşitlilik).
  **Sıradaki adım:** Proje sahibinden Parşömen temasında Kitaplık ekranına bakması istendi — sistemik bir hata varsa 19 dosyada tekrarlamadan burada yakalanır. Onay gelirse sıradaki dosyalar aynı şablonla devam eder (öncelik: `main.dart`'ın Dashboard'ı, `flashcards_screen.dart`, `leaderboard_screen.dart`, `profile_screen.dart`, `dictionary_screen.dart`).

### Beşinci tur — "Parşömen sadece Kitaplığı değiştiriyor" bildirimi, T-1 hızlandırıldı (19.09.2026 devam)

Proje sahibi Parşömen geçişinin hâlâ sadece Kitaplık'ı etkilediğini, seçimin Profil'den yapılmasının kafa karıştırıcı olduğunu bildirdi ve kalan her şeyin sorulmadan hızlıca tamamlanmasını istedi. Bu, gerçek bir hata değil — T-1 henüz dosya dosya sürmekteydi; aşağıdaki dosyalar da tamamlanarak devam edildi (hepsi `library_screen.dart`'taki aynı şablonla, aynı bilinçli-dokunulmayan istisnalarla: bespoke gradient "hero" kartlar — main.dart'ın "Günün Görevi"si, flashcards'ın Memory Dungeon'ı, leaderboard/profile'ın Lig/Profil kartı —, sabit koyu zeminli bottom sheet/dialog'lar, ve kart/rozet/accentColor gibi kasıtlı çok-renkli semantik paletler):

- [x] **`main.dart`** ✅ — Dashboard, `RootScreen`'in paylaşılan Scaffold + BottomNavigationBar'ı dahil (tüm 5 sekmede etkili).
- [x] **`flashcards_screen.dart`** ✅ — Arena ekranı; Ayarlar bottom sheet'i (sabit koyu zemin) ve Memory Dungeon hero kartı bilinçli olarak dokunulmadı.
- [x] **`leaderboard_screen.dart`** ✅ — tam yapısal geçiş (önceki turda sadece C-1/N-5/P0-C/C-2 noktasal düzeltmeleri vardı); Lig özet kartı (bespoke gradient) dokunulmadı.
- [x] **`profile_screen.dart`** ✅ — **Görünüm anahtarının kendisinin bulunduğu ekran.** Zindan/Parşömen segmented control'ü artık kendisi de tema-farkında; rozet vitrini, istatistik listesi, Ayarlar bölümü, Lig kartı tam geçti. Rozet detay + Çerçeve seçici bottom sheet'leri (sabit koyu zemin) ve Profil hero kartı bilinçli dokunulmadı.
- [x] **`dictionary_screen.dart`** ✅ — tam yapısal geçiş (önceki turda sadece tek etiket düzeltmesi vardı); silme onay dialog'u (sabit koyu zemin) dokunulmadı.
- [x] **`habit_tracker_screen.dart`** ✅ — "Bugünkü İlerleme" hero kartı (main.dart'ın "Günün Görevi"siyle aynı desen) ve alışkanlık ekle/düzenle dialog'ları (sabit koyu zemin) dokunulmadı.
- [x] **`book_journey_screen.dart`** ✅ — Kelime Dağılımı bottom sheet'i (sabit koyu zemin) dokunulmadı; "Sonraki Hedef" kartının kasıtlı indigo-tonlu zemin/kenarlığı da (Vocabulary Journey'nin "bilişsel" rengiyle tutarlı bir marka kararı) semantik olarak korundu, sadece iç genel metinleri tema-token'ına taşındı.
- [x] **`shop_screen.dart`** ✅ (73KB, en büyük dosya) — Scaffold/sticky sekme çubuğu, envanter barı, sekme chip'leri, kalkan uyarı banner'ı, bahis kartı, bölüm başlıkları, kozmetik gardırop kartı ve tüketilebilir kartların yapısal zemin/kenarlık/metinleri taşındı. **Bilinçli dokunulmadı:** "Destansı Sandık" vitrin kartı (kasıtlı canlı pembe/indigo promosyon kartı), elmas yetersiz dialog'u ve sandık açılış töreni animasyon sınıfları (ayrı, tam ekran kutlama widget'ları — celebration_dialog.dart ile aynı kategori), çerçeve önizleme avatarları (Profil'deki `_buildCosmeticAvatar` ile bilerek birebir), ve her kozmetik/tüketilebilir öğenin kendi sabit `iconColor`/rozet paleti.
- **`app_header.dart` ve `celebration_dialog.dart` — T-1'e HİÇ ihtiyaçları yok, incelendi ve atlandı.** İkisi de zaten kendi başına `Theme.of(context).brightness` kontrolü yapıp koyu/açık için ayrı hex çiftleri seçiyor; `main.dart`'taki `MaterialApp`'in `brightness`'ı `draconicTheme.isDark`'a bağlı olduğu doğrulandı — yani bu iki dosya Zindan/Parşömen'e halihazırda doğru tepki veriyor, farklı bir mekanizmayla (DraconicTheme extension yerine ham Brightness).
- **`reader_screen.dart` — T-1 KAPSAMI DIŞI, kasıtlı olarak.** Kendi bağımsız 9 temalı okuma paleti var (`ReaderTheme`: light/sepia/dark/neon/parchment/nordic/espresso/oled/sakura) — bu, Kindle/Apple Books tarzı, uygulama temasından tamamen ayrı, kasıtlı bir "okuma modu" katmanı. DraconicTheme'e bağlamak bu tasarımı bozar; hiç dokunulmadı.

Her commit, standart yöntemle doğrulandı (stage → edit → commit → force-retry (ilk deneme neredeyse her dosyada sessizce başarısız oldu) → re-stage ile byte sayısı karşılaştırması).

**Kalan T-1 kapsamı (henüz dokunulmadı):** egzersiz ekranları (`quiz_exercise_screen.dart`, `match_exercise_screen.dart`, `spelling_exercise_screen.dart`, `cloze_exercise_screen.dart`, `reverse_quiz_screen.dart`, `listening_exercise_screen.dart`, `speed_round_screen.dart`, `mixed_dungeon_session_screen.dart`, `word_boss_battle_screen.dart`, `flashcards_exercise_screen.dart`), `design_lab_screen.dart` (muhtemelen geliştirici-amaçlı, kapsam dışı olabilir — teyit edilmedi). Sıradaki oturumda dosya dosya devam edilecek; her biri için önce `app_header.dart`/`celebration_dialog.dart`'ta olduğu gibi zaten Brightness-farkında olup olmadığı kontrol edilecek (T-1'i tekrar yapmamak için).
...
- **RPG temasını geri çekip eğitimi öne çıkarma (19.09.2026, kullanıcı notu).** Uygulama fazla "oyun" hissi veriyor ve tasarımda kafa karıştırıcı olmaya başladı; hedef, oyun temasını biraz arkaya alıp eğitim/öğrenme tarafını daha öne çekmek. Şimdilik sadece not — hangi ekranların/öğelerin "fazla oyun" sayıldığı, ne kadar geri çekileceği henüz tartışılmadı. **Ne zaman dönülür:** T-1 (tema migrasyonu) tamamlandıktan ve mevcut kullanıcı geri bildirimi toplandıktan sonra, ayrı bir tasarım turu olarak.
- **Aydınlık (Parşömen) temanın artık zorunlu/öncelikli hale gelmesi (19.09.2026, kullanıcı notu).** Yukarıdaki RPG-geri-çekme kararıyla bağlantılı: aydınlık tema artık "ek özellik" değil, tamamlanması gereken bir gereklilik olarak görülüyor. T-1 (ekran ekran `DraconicTheme` migrasyonu) bu kararın önkoşulu — şu an ilerlemekte olan iş zaten bu yöne hizmet ediyor.
- **Bu uygulama tam çıktıktan sonra bir "eğitim uygulamaları ekosistemi" kurma fikri (19.09.2026, kullanıcı notu).** Ignis'teki RPG'li eğitim yaklaşımını farklı konular/ihtiyaçlar için ayrı uygulamalara genişletme planı — uzun vadeli, Ignis'in kendisi tamamlanmadan başlanmayacak. Şu an aksiyon gerektirmiyor, sadece gelecekteki yön kararı olarak not edildi.
- **Firebase ile hesap oluşturma + lige XP üzerinden bağlı senkron (19.09.2026, kullanıcı notu).** Yukarıdaki "Bulut hesabı / Supabase yedekleme" kararıyla örtüşüyor ama farklı bir motivasyonla geldi: hesap sistemi artık liderlik tablosunu gerçek/senkronize kılmak için isteniyor (bkz. "Sahte sosyal kanıt kararı"). Detaylar henüz konuşulmadı — kullanıcının kendi ifadesiyle "bunun detaylarını konuşuruz, şimdilik aklında bulunsun". **Ne zaman dönülür:** Supabase/hesap kararına dönüldüğünde bu not da birlikte ele alınmalı, çünkü aynı alt yapıyı (kullanıcı kimliği) paylaşıyorlar.
