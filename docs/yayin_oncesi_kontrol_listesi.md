...
**Aşama 3 durumu: 4/4 mod tamamlandı** (Cümlede Boşluk Doldurma ücretsiz, Ters Test / Sadece Dinleme / Hız Turu Premium). `flutter analyze` kullanıcı tarafından teyit edilmeli; ardından Arena grid'i artık 2x2 değil 2 sütun × 4 satır (8 kart) — Aşama 4'te tasarım toparlaması sırasında düzen yeniden değerlendirilebilir (ör. yatay kaydırılabilir liste veya kategorilere ayırma).
